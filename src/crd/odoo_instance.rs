...
pub struct FilestoreSpec {
    ...
    /// Run the filestore on a per-pod `emptyDir` instead of a
    /// PersistentVolumeClaim.
    ///
    /// Setting this is declaring that the filestore holds nothing worth
    /// preserving: attachments live in external object storage (e.g. an OCA
    /// `fs_attachment` S3 backend) and sessions in a non-filesystem store, so
    /// what remains on disk is regenerable (asset caches, temp files). It is
    /// the operator user's responsibility to have moved attachments and
    ...
    ///
    /// The field is mutable in both directions.
    #[serde(default)]
    pub empty_dir: bool,
}
